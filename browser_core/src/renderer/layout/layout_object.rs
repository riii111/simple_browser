...
impl LayoutObject {
    ...
    pub fn create_layout_object(
        ...
        if let Some(n) = node {
            ...
            // CSSでスタイルが指定されていない場合、デフォルト値または親ノードから継承した値を使用
            ...
            // displayプロパティがnoneの場合、ノードを生成しない
            if layout_object.borrow().style().display() == DisplayType::DisplayNone {
                return None;
            }

            // displayプロパティの最終的な値を用いてノードの種類を決定
    ...
    /// CSSルールをレイアウトツリーのノードに適応すべきか判断するメソッド
        ...
        match &self.node_kind() {
            NodeKind::Element(e) => match selector {
                ...
                Selector::IdSelector(id_name) => {
                    for attr in &e.attributes() {
                        if attr.name() == "class" && attr.value() == *id_name {
                            return true;
                        }
                    }
                    false
    ...
    /// CSSの宣言リスト（declarations）を引数に、各宣言のプロパティをノードに適応するメソッド
    ...
    pub fn update_kind(&mut self) {
        match self.node_kind() {
            NodeKind::Document => panic!("should not crate a layout object for a Document node"),
            NodeKind::Element(_) => {
    ...
    /// ノードにCSSの値が明示的に指定されていない場合は、指定値を使用する
